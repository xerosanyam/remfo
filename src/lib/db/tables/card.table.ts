...
export const getCardsOrderByCreated = async (userId: string, limit: number) => {
	...
	const data = await db
		...
		.from(cardTable)
		.where(and(eq(cardTable.userId, userId), eq(cardTable.deleted, false)))
...
export const getDailyActivity = async (userId: string) => {
	...
	const data = await db
		.select({ action: activityTable.action, date, count: count() })
		.from(activityTable)
		.where(eq(activityTable.userId, userId))
		.groupBy(activityTable.action, date);
...
export const getCardsGroupedByActivityDate = async (userId: string) => {
	...
	const data = await db
		.select({ date: sql`DATE(${activityTable.createdAt}, 'unixepoch')`, count: count() })
		.from(activityTable)
		.where(eq(activityTable.userId, userId))
		.groupBy(sql`DATE(${activityTable.createdAt}, 'unixepoch')`);
